...
def run(
    ...
    """Execute the full ChIP-seq pipeline."""
    # 1. Alignment
...
def _peaks(spec: JobSpec, paths: RunPaths, log_file: Path) -> None:
    if spec.do_calibration:
        sample_bam = paths.sample_non_aligned_calibration_aligned_genome_filtered
    ...
    call_peaks(spec, sample_bam, out_dir, peak_name, log_file, input_bam=input_bam)
